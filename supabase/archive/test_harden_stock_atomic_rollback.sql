...
-- ============================================================
-- 1) Contenido completo de supabase/harden_stock_atomic.sql
--    (sin BEGIN/COMMIT propios: ya no los tiene el archivo
--    original, se reproduce tal cual dentro de esta transaccion
--    de prueba para que el ROLLBACK final deshaga tambien el
...
-- ============================================================
-- 2) Prueba completa del RPC contra la version de create_order_v2
--    que se acaba de crear arriba, dentro de esta misma
--    transaccion. Tablas temporales calificadas explicitamente
--    como pg_temp.* en toda referencia.
-- ============================================================

create temporary table pg_temp.test_ctx as
select
...
from pg_temp.test_ctx tc
where cv.sanity_product_id = tc.sanity_product_id
	and cv.sku = tc.sku;

create temporary table pg_temp.test_results (step text, result text);
...
begin
	...
	into v_pid, v_sku, v_pm, v_dm from pg_temp.test_ctx;
	...
	insert into pg_temp.test_results values ('1_compra_normal', 'order=' || v_order);
exception when others then
	insert into pg_temp.test_results values ('1_compra_normal', 'ERROR: ' || sqlerrm);
end $$;

insert into pg_temp.test_results
select '1_stock_tras_compra', stock_quantity::text
from public.catalog_variants cv join pg_temp.test_ctx tc
...
begin
	...
	insert into pg_temp.test_results values ('2_misma_clave_repetida', 'order=' || v_order);
exception when others then
	insert into pg_temp.test_results values ('2_misma_clave_repetida', 'ERROR: ' || sqlerrm);
end $$;
...
-- 3) Clave distinta pidiendo mas del disponible (quedan 9, pide 10) ->
--    stock insuficiente. Cantidad 10 (no 100): el maximo permitido por
--    item es 50, y 100 hubiera chocado con ese limite en vez de probar
--    el rechazo por stock insuficiente.
do $$
declare v_pid text; v_sku text; v_pm public.payment_method; v_dm public.delivery_method; v_order text;
begin
	select sanity_product_id, sku, payment_method, delivery_method
	...
	insert into pg_temp.test_results values ('3_clave_distinta_excede_stock', 'order=' || v_order);
exception when others then
	insert into pg_temp.test_results values ('3_clave_distinta_excede_stock', 'ERROR: ' || sqlerrm);
end $$;

insert into pg_temp.test_results
select '3_stock_tras_rechazo', stock_quantity::text
from public.catalog_variants cv join pg_temp.test_ctx tc
	on cv.sanity_product_id = tc.sanity_product_id and cv.sku = tc.sku;
...
begin
	...
	insert into pg_temp.test_results values ('4_misma_clave_datos_distintos', 'order=' || v_order);
exception when others then
	insert into pg_temp.test_results values ('4_misma_clave_datos_distintos', 'ERROR: ' || sqlerrm);
end $$;
...
begin
	...
	insert into pg_temp.test_results values ('5_segundo_item_falla', 'order=' || v_order);
exception when others then
	insert into pg_temp.test_results values ('5_segundo_item_falla', 'ERROR: ' || sqlerrm);
end $$;

-- 6) Comprobacion final: el stock temporal debe quedar en 9 (solo el
--    descuento del caso 1 quedo en pie; casos 2 a 5 no debieron tocarlo).
insert into pg_temp.test_results
select '6_stock_final_esperado_9', stock_quantity::text
from public.catalog_variants cv join pg_temp.test_ctx tc
	on cv.sanity_product_id = tc.sanity_product_id and cv.sku = tc.sku;

select * from pg_temp.test_results order by step;
